...
public class DAOTipoComida extends DAO<TipoComida> {
	...
	public TipoComida read(Object chave) {
		try {
			...
			TypedQuery<TipoComida> query = manager.createQuery("select t from TipoComida t where t.nome=:nome",
					TipoComida.class);
			query.setParameter("nome", nome);
			TipoComida tipoComida = query.getSingleResult();
			return tipoComida;
	...
	public List<TipoComida> readAll() {
		TypedQuery<TipoComida> query = manager.createQuery("select t from TipoComida t order by t.nome",
				TipoComida.class);
